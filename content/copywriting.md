# Copywriting banka — Alex Liška, freelance web developer

Vykání, čeština. Přeloženo a upraveno ze starého (anglického) portfolia + doplněno o nový obsah pro vícestránkový web (Úvod / O mně / Portfolio / Služby / Blog). Ber to jako zásobník textu — vyber, zkrať, kombinuj podle sekce, nemusíš použít doslova všechno.

---

## 1. Úvod (Home)

**H1 — varianty:**
- Tvorba webových aplikací — Pardubice a okolí *(už používáš)*
- Weby a webové aplikace na míru — Pardubice a okolí
- Freelance webový vývojář z Pardubic

**Podtitulek / bio (krátká verze, pod H1):**
> Freelance webový vývojář s full-stack zázemím. Stavím landing pages, firemní weby i webové aplikace — a záleží mi na detailech, od architektury backendu až po to, jak se web chová při scrollování.

**Ještě kratší (na úvodní obrazovku, pod H1 jako jedna řádka):**
> Full-stack vývoj webů a webových aplikací na míru.

**Scroll cue (mikrotext):**
- `---- [ scroluj ] ----`
- `↓ scroluj ↓`

**Booking / dostupnost (badge nebo řádek u CTA):**
> Aktuálně přijímám nové projekty

**CTA tlačítka:**
- Domluvit konzultaci
- Poptat projekt
- Prohlédnout portfolio
- Napsat mi

---

## 2. O mně (About)

**Nadpis:**
- Alex Liška — Freelance developer *(máš)*
- Kdo stojí za tím webem

**Delší bio (odstavec 1 — kdo jsem):**
> Jsem freelance webový vývojář z Pardubic a okolí. Weby a webové aplikace stavím na míru — od jednoduché prezentace po plnohodnotnou webovou aplikaci s vlastním backendem, databází a administrací.

**Odstavec 2 — proč full-stack / odkud jdu:**
> Ke kódování jsem se dostal jako samouk, souběžně studuji mechatroniku na TUL (a předtím obor Technik informačních systémů). Ta kombinace — technické vzdělání a praxe s automatizací a řídicími systémy — je vidět i na tom, jak k webu přistupuju: záleží mi na tom, aby fungoval spolehlivě, ne jen aby vypadal dobře.

**Odstavec 3 — přístup k práci:**
> Nemám rád zbytečný žargon ani nekonečné revize. Domluvíme si rozsah, cenu a termín předem, a Vy víte, co dostanete a kdy. Práci nad rámec zadání účtuju férově a hodinově — žádná skrytá překvapení ve faktuře.

**Krátká verze (pro sidebar/kartu, když je odstavců moc):**
> Freelance webový vývojář z Pardubic. Full-stack pozadí, technické vzdělání (mechatronika, informační systémy) a smysl pro detail — od backendu po to, jak se stránka chová při scrollu.

**Vzdělání (Vzdělání / Education):**
| Období | Co | Kde | Popis |
|---|---|---|---|
| 2025 – nyní | Vývoj webových aplikací | samouk | Stavím reálné projekty s Reactem, Node.js a MongoDB. |
| 2024 – nyní | Mechatronika | TUL | Robotika, automatizace a řídicí systémy. |
| 2020 – 2024 | Technik informačních systémů | SOŠ Kolín | Základy IT, elektronika, mikrokontroléry, zabezpečovací technika. |

**Praxe (Experience):**
| Období | Co | Kde | Popis |
|---|---|---|---|
| 06.2024 – 08.2024 | Údržba výrobní linky | Kiekert | Stavba a demontáž výrobních linek. |
| 06.2023 – 08.2023 | Údržba a sklad | Kiekert | Správa skladu a podpora linky. |
| 05.2022 | Výroba rozvaděčů | Manag | Dvoutýdenní stáž přímo ve výrobě. |
| 05.2021 | Úvod do výroby | Manag | První praktický kontakt s průmyslovou výrobou. |

*(poznámka pod sekcí: „Kompletní životopis na vyžádání.")*

**Tech stack — nadpisy sekcí:**
- Backend technologie
- Frontend technologie

**Proč se mnou (USP bullets — hodí se i na Úvod nebo Služby):**
- Full-stack pod jednou střechou — nepotřebujete koordinovat tři různé dodavatele
- Pevná cena předem, žádná překvapení ve faktuře
- Technické vzdělání = smysl pro detail a spolehlivost, ne jen hezký vzhled
- Komunikace bez žargonu — vysvětlím, co a proč děláme
- Rychlá odezva, jasné termíny

---

## 3. Portfolio

**Mikrotext nad sekcí:**
- `[ vybrané práce ]`
- `[ vybrané projekty ]`

**Prázdný stav / placeholder pro další projekty:**
> [ další už brzy ]

**Filtry (typ projektu) — pokud chceš lokalizovat `TYPE_LABEL`:**
- Personal → Osobní
- Freelance → Freelance
- Work → Zaměstnání *(nebo nechat „Práce")*
- All → Vše

**Modal (detail projektu):**
- Zavřít detail projektu *(aria-label)*
- Zobrazit projekt →
- `[ zobrazit ]`
- Technologie *(stack)*
- Odkazy *(links)*

**Popisky ukázkových projektů (vzor, uprav podle skutečných):**
- CLI nástroj — „CLI nástroj pro rychlé zakládání rozpracovaných projektů."
- Sledování tréninků — „Gamifikovaná aplikace pro sledování tréninků se zaměřením na kalisteniku. Pomáhá s plynulým postupem ke složitějším cvikům."

---

## 4. Služby (Services / Pricing)

**Nadpis sekce (dvouřádkový styl jako u originálu):**
- Spolupráce / se mnou
- Pojďme / spolupracovat

**Mikrotext:**
> [ pevný rozsah · ceny od · žádná překvapení ]

**Poznámka pod balíčky:**
> Základní ceny — finální cena závisí na tom, jaké funkce skutečně potřebujete.

**Když si nejsou jistí:**
> Nejste si jistí, co potřebujete? Napište mi e-mail.

**Obecné labely (pro kalkulačku):**
- od (from)
- odhad (est.)
- začátek (start)
- / měs. (monthly suffix)
- cena od (from-price scope)
- měsíčně (monthly scope)

### Balíček 1 — Malý web
**Cena:** od **12 000 Kč**
**Tagline:** landing page nebo web do 3 stránek
**Termín:** 1–2 týdny od dodání podkladů
**Zahrnuje:**
- design + vývoj, plně responzivní (úvodní stránka cca 5 sekcí)
- až 3 stránky celkem (např. Úvod, Služby, Kontakt)
- základní on-page SEO (title, meta, nadpisy, sitemapa, OG tagy)
- kontaktní formulář (Formspree/EmailJS nebo lehký vlastní endpoint)
- nasazení na Cloudflare Pages + nastavení vlastní domény (hosting v ceně u balíčku správy, jinak předáno na Váš účet)
- základní animace (plynulý scroll přes Lenis + pár GSAP přechodů)
- obsah nasazuji já — administrace není součástí, lze přidat jako doplněk
- odkaz na autora v patičce („web od…") — odstranění za příplatek
- texty a fotografie dodává klient — copywriting je volitelný doplněk
- 2 kola připomínek v ceně, další úpravy hodinově

**Doplňky:**
- stránka navíc (nad rámec 3) — **2 000 Kč / stránka**, max. 3 stránky navíc
- administrace (vlastní hostovaný CMS — Strapi) — plná kontrola nad obsahem (vyžaduje hosting backendu) — **8 000 Kč** *(nelze kombinovat s cloudovým CMS)*
- administrace (cloudový CMS — Sanity apod.) — bez vlastního serveru, běží u poskytovatele — **2 800 Kč** *(nelze kombinovat s vlastním hostovaným CMS)*
- druhá jazyková verze — **3 000 Kč**
- animace / motion na míru — **3 500 Kč**
- pokročilé SEO (jednorázové nastavení) — **2 500 Kč**
- nastavení analytiky (GA4 + cíle) — **1 500 Kč**
- copywriting — **3 000 Kč**
- odstranění odkazu „web od…" v patičce — **1 500 Kč**

### Balíček 2 — Firemní web (Business Website)
**Cena:** od **25 000 Kč**
**Zaváděcí cena:** **16 000 Kč** pro první 3 zakázky — výměnou za referenci, souhlas se zveřejněním v portfoliu a krátký citát. Platí do naplnění tří míst; pak se položka `intro` v `data/pricing.ts` smaže a ceník zůstává nedotčený.
**Tagline:** vícestránkový web pro Vaši firmu
**Termín:** 2–4 týdny od dodání podkladů
**Zahrnuje:**
- vše z balíčku Malý web
- až 5 podstránek (Úvod, O nás, Služby, Reference, Kontakt) — další za příplatek
- formulář s backendem (ukládání do databáze / zasílání e-mailem)
- nastavení analytiky a Search Console
- prémiové animace (GSAP + Lenis)
- obsah nasazuji já — administrace je volitelný doplněk
- předání přístupů + zaškolení
- odkaz na autora v patičce — odstranění za příplatek
- texty a fotografie dodává klient — copywriting je volitelný doplněk
- 2 kola připomínek v ceně, další úpravy hodinově

**Doplňky:**
- administrace (vlastní hostovaný CMS — Strapi) — novinky a reference; backend běží na vlastním serveru, měsíční hosting v ceně doplňku správy — **9 000 Kč** *(nelze kombinovat s cloudovým CMS)*
- administrace (cloudový CMS — Sanity apod.) — bez vlastního serveru — **4 000 Kč** *(nelze kombinovat s vlastním hostovaným CMS)*
- stránka navíc (nad rámec 5) — **2 500 Kč / stránka**, max. 5 stránek navíc
- další jazyk — **5 000 Kč**
- jednoduchý katalog produktů — **15 000 Kč**
- integrace rezervačního systému (Calendly apod.) — **10 000 Kč**
- pokročilé SEO a schema (jednorázové nastavení) — **4 000 Kč**
- audit přístupnosti (WCAG) — **3 500 Kč**
- copywriting — **4 500 Kč**
- integrace newsletteru — **2 500 Kč**
- integrace třetích stran (mapy/chat/sociální sítě) — **4 000 Kč**
- odstranění odkazu „web od…" — **3 000 Kč**

### Balíček 3 — Webová aplikace (MVP)
**Cena:** od **45 000 Kč**
**Tagline:** full-stack MVP na Next.js — cena po úvodní analýze
**Termín:** 4–8 týdnů od dodání podkladů
**Zahrnuje:**
- úvodní analýza rozsahu — teprve z ní vzejde pevná cena a termín
- autentizace (přihlášení / registrace)
- databáze + základní CRUD / business logika
- backend / API na míru (Next.js route handlers, případně samostatný Node backend)
- administrační rozhraní nebo dashboard
- responzivní UI + nasazení se základním CI/CD
- texty a fotografie dodává klient — copywriting je volitelný doplněk
- 2 kola připomínek v ceně, další úpravy hodinově

**Doplňky:**
- platební integrace — **20 000 Kč**
- role a oprávnění — **8 000 Kč**
- e-mailové/push notifikace — **6 000 Kč**
- reporty, analytika a export dat — **10 000 Kč**
- integrace API třetích stran — **12 000 Kč**
- real-time funkce — **15 000 Kč**
- podpora PWA — **7 000 Kč**
- sada automatizovaných testů — **8 000 Kč**

### Balíček 4 — Správa a údržba (Maintenance)
**Tagline:** web zůstává v provozu, aktuální a zálohovaný
**Termín:** průběžně
**Zahrnuje:**
- hosting frontendu v ceně (Cloudflare Pages) — žádná další faktura za provoz webu
- aktualizace závislostí a bezpečnostní záplaty
- monitoring dostupnosti (uptime) — když web spadne, řeším to já, klient si toho nemusí všimnout
- zálohy včetně ověřeného obnovení — web zpět v provozu do 24 h
- opravy chyb v mém vlastním kódu zdarma po celou dobu správy
- hlídání expirace domény — registraci platí klient přímo registrátorovi, doména je psaná na něj
- úpravy textů cookie lišty a GDPR podle změn legislativy
- pro weby postavené mnou (jiné po dohodě)

**Úrovně:**
| Úroveň | Cena | Popis |
|---|---|---|
| Základ | **600 Kč / měs.** | hosting frontendu, aktualizace a záplaty, monitoring dostupnosti, zálohy — a opravy chyb v mé práci zdarma |
| Standard | **1 200 Kč / měs.** | vše ze Základu + odpověď do 24 h v pracovní dny, drobné úpravy obsahu a měsíční report (dostupnost, návštěvnost, co jsem udělal) |
| Aplikace | **2 400 Kč / měs.** | pro weby s backendem — vše ze Standardu + správa serveru, monitoring chyb a prioritní fronta |

**Doplňky:**
- hosting CMS/backend serveru — **700 Kč / měs.** *(provozní náklad serveru, účtuje se navíc u všech úrovní včetně Aplikace)*
- správa serveru (aktualizace běhového prostředí, restarty, zálohy databáze) — **700 Kč / měs.** *(už zahrnuto v úrovni Aplikace)*
- prioritní podpora — **400 Kč / měs.** *(už zahrnuto v úrovni Aplikace, nelze přidat navíc)*
- hodina úprav navíc měsíčně — **600 Kč / měs.**

### Kalkulačka / konfigurátor — mikrotexty
- Zavřít konfigurátor
- Výchozí cena — finální cena je na domluvě podle toho, jaké funkce skutečně potřebujete.
- Volitelné doplňky
- Odhadovaná celková cena
- Odráží vybrané doplňky — plně individuální zadání může finální cenu upravit.
- Vyberte si úroveň
- Práce nad rámec dohodnutého zadání se účtuje hodinově (~600 Kč/h) · záloha 50 % předem.
- Termín běží od dodání podkladů (texty, fotky, přístupy), ne od objednávky. V ceně jsou 2 kola připomínek, další úpravy hodinově.
- Co je součástí
- / stránka
- Doména se registruje na klienta a platí ji přímo registrátorovi — zůstává jeho. Ve správě mu hlídám datum expirace. Placené služby třetích stran (rezervační systém, platební brána…) nejsou součástí ceny.
- Hosting frontendu je v ceně správy. Administrace (CMS) a backend aplikace potřebují vlastní server navíc — doplněk správy, u úrovně Aplikace už zahrnutý.
- Odebrat stránku / Přidat stránku

### Poptávkový formulář
- Jméno
- E-mail
- Zpráva
- Placeholder zprávy: *co budeme stavět?*
- Odeslat
- Odesílám…
- Odesláno — brzy se ozvu
- Něco se pokazilo — zkuste to znovu nebo mi napište přímo na e-mail
- Odeslat poptávku s touto konfigurací
- Předmět (s balíčkem): *Poptávka z webu — {{service}}*
- Předmět (obecný): *Zpráva z webu*

---

## 5. Extra obsah pro vícestránkový web

Jednostránkový web si vystačí s málem, ale samostatné stránky potřebují víc textu, aby nepůsobily prázdně a dávaly smysl i pro SEO. Pár bloků navíc, co starý web neměl:

### Jak probíhá spolupráce (hodí se na Služby nebo O mně)
1. **Poptávka a konzultace** — napíšete mi přes formulář nebo e-mail, co potřebujete, a probereme rozsah, cíle i rozpočet.
2. **Návrh a nacenění** — dostanete konkrétní nabídku s pevnou cenou a termínem; u webových aplikací až po úvodní analýze rozsahu.
3. **Předání podkladů** — pošlete texty, fotky a přístupy. Teprve tímhle krokem začíná běžet termín.
4. **Vývoj** — stavím web, průběžně vidíte progres.
5. **Předání a zaškolení** — dostanete přístupy, administraci a případně zaškolení.
6. **Podpora** — volitelně navazuje balíček Správa a údržba.

### FAQ — Časté dotazy (Služby)
- **Jak dlouho trvá tvorba webu?** Podle rozsahu — malý web 1–2 týdny, firemní web 2–4 týdny, webová aplikace 4–8 týdnů. Termín běží od dodání podkladů, ne od objednávky.
- **Kdo píše texty a dodává fotky?** Standardně klient. Copywriting je volitelný doplněk u každého balíčku.
- **Kolik mám kol připomínek?** Dvě kola v ceně u všech balíčků, další úpravy hodinově (~600 Kč/h).
- **Co když přesně nevím, co potřebuju?** Není problém, na to je úvodní konzultace — společně to vyladíme.
- **Poskytujete hosting?** Ano, v rámci balíčku Správa a údržba. Bez něj hosting předávám na Váš účet.
- **Jak probíhá platba?** Záloha 50 % předem, zbytek po předání. Práce nad rámec zadání se účtuje hodinově.
- **Budu moct web upravovat sám?** Podle toho, jestli si přiberete administraci (CMS) jako doplněk. S ní si novinky, reference a texty upravujete sami a zaškolím Vás. Bez ní obsah nasazuji já.
- **Proč není administrace rovnou v ceně?** Protože ji velká část malých firem nikdy nepoužije. Je to nejdražší část webu na výrobu i na následnou podporu — férovější je nechat rozhodnutí na klientovi.
- **Za jak dlouho odpovíte na poptávku?** Do 24 hodin v pracovní dny.
- **Co po předání, když najdu chybu?** Opravy v rámci dohodnutého rozsahu jsou součástí revizí, cokoliv navíc řešíme v rámci správy nebo hodinově.

### Blog — úvodní mikrotext (než tam bude obsah)
> Zatím tu nic není, ale brzy tu najdete tipy k tvorbě webů, poznámky z praxe a rozbory vlastních projektů.

Náměty na kategorie/první příspěvky:
- Jak vybrat mezi landing page a firemním webem
- Co všechno je „základní SEO" a proč na tom záleží
- Proč používám Cloudflare Pages místo klasického hostingu
- Mechatronika vs. programování — co mi jedno dalo pro to druhé

### Meta / SEO texty (title + description) — návrh na stránku
- **Úvod:** *Tvorba webových stránek | Alex Liška — Pardubice a okolí* *(máš)* — description: „Freelance webový vývojář z Pardubic. Landing pages, firemní weby a webové aplikace na míru — pevná cena, jasný termín."
- **O mně:** *O mně | Alex Liška — freelance webový vývojář* — description: „Full-stack vývojář s technickým vzděláním (mechatronika, informační systémy). Poznejte, kdo stojí za Vaším projektem."
- **Portfolio:** *Portfolio | Alex Liška* — description: „Vybrané projekty — osobní, freelance i pracovní. Weby a webové aplikace postavené na míru."
- **Služby:** *Služby a ceník | Alex Liška* — description: „Malý web, firemní web, webová aplikace nebo správa webu — ceny od, kalkulačka a poptávkový formulář."

### Patička (Footer) — pokud budeš chtít jednotnou patičku napříč stránkami
- Pozdravte se
- Pojďme spolu / postavit něco skvělého
- github / linkedin / e-mail
- Pardubice, Česká republika
- © {{rok}} Alex Liška — všechna práva vyhrazena
- zpět nahoru ↑
- IČO: 29805473
- E-mail: alex.liska04@seznam.cz

---

## Poznámky
- Vykání používej všude, kde oslovuješ návštěvníka přímo („napište mi", „nejste si jistí", „dostanete"). Popisy o sobě zůstávají v 1. osobě jednotného čísla.
- Ceny doplňků, úrovní správy i základní ceny „od" u všech čtyř balíčků jsou doplněné (Malý web 12 000 Kč, Firemní web 25 000 Kč / zaváděcích 16 000 Kč pro první 3 zakázky, MVP 45 000 Kč, Správa od 600 Kč/měs. včetně hostingu frontendu).
- `cms` a `cms_hosted` (Landing) se vzájemně vylučují — v UI/copy by mělo jít vybrat jen jedno z toho.
- `extra_page` (Firemní web) je množstevní doplněk, max. 5 stránek navíc — v kalkulačce se tedy chová jako počitadlo, ne zaškrtávátko.
- `priority_support` (Správa) nejde přidat k úrovni Aplikace — tam už je zahrnutá.
- `TYPE_LABEL` a filtr v `app/portfolio/page.tsx` jsou zatím natvrdo v angličtině (Personal/Freelance/Work) — pokud chceš celý web česky, budeš je muset přepnout v kódu, tenhle soubor jen navrhuje překlad.
